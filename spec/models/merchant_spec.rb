...
describe Merchant, type: :model do
  ...
  describe ".revenue_within_dates(start, end)" do
    it "grabs the merchants who have sold the most items" do
      ...
      invoices[5..8].each{ |invoice| create(:transaction, invoice_id: invoice.id, result: 'success')}
      expect(Merchant.revenue_within_dates("1990-01-01", "2022-03-24")).to eq(invoices[5..8].flat_map{ |invoice| invoice.invoice_items}.sum{ |i| i.quantity * i.unit_price})
    end
  end
  ...
  #     end
  #
  #     expect(Merchant.revenue(Merchant.first[:id])).to eq(InvoiceItem.sum('quantity * unit_price'))
  #
  #
  #     invoices[5..8].each do |invoice|
  #       create(:invoice_item, invoice_id: invoice.id, quantity: x += 300, unit_price: 10)
